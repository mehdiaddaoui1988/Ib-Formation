...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("button-try")!.addEventListener("click", () => {
    // LA VALEUR DU NOMBRE TENTE LORS DU CLIC
    var nombreTente = (document.getElementById("text-try")! as HTMLInputElement)
      .value;

    // SI ON RENTRE UNE VALEUR /////////////////VERIFIER CHIFFRE
    if (
      (document.getElementById("text-try")! as HTMLInputElement).value != ""
    ) {
      // AJOUTE UNE LIGNE A LA TABLE
      var newLine = (document.getElementById(
        "table-historique"
      )! as HTMLTableElement).insertRow();
